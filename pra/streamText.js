// ==UserScript==
// @name         Streaming out the texts
// @namespace    http://tampermonkey.net/
// @version      0.0.2
// @description  ctr + shift + k  for activate
// @author       dcthehiker
// @match        *://*/*
...
(function() {
    ...
    // setting
    const pace = 10;// how many words underlined
    let delay = 300;// time delayed
    const min_delay = 50; // faster
    const max_delay = 1000; // slower
    ...
    async function streaming(textNode) {
        const text = textNode.textContent;
        const parent = textNode.parentElement;

        // parent can display texts
        parent.style.color = '';
        ...
        for (let i=0;i < segs.length;i++){
            // hide texts
            segs[i].style.color = 'transparent';
        }
    ...
    async function processAllTextNodes(dom_js) {
        ...
        for (const textNode of textNodes){
            textNode.parentElement.style.color = 'transparent';
        }
        for (const textNode of textNodes) {
            //await animateUnderline(textNode);
            await streaming(textNode);
        }
    }

    // inspect dom  do underline when click
